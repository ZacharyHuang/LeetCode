...
typedef struct transaction tran;
struct transaction
{
	int b;
	int s;
...
// unsafe stack
typedef struct transtack transtack;
struct transtack
{
	tran* data;
	int size;
...
transtack* TransStackCreate(int size)
{
	transtack* stack = malloc(sizeof(transtack));
	stack->data = malloc(size * sizeof(tran));
	stack->size = size;
	stack->length = 0;
	return stack;
}

void TranStackPush(transtack* stack, int b, int s)
{
	stack->data[stack->length].b = b;
	stack->data[stack->length].s = s;
	++stack->length;
}
#define Push TranStackPush

tran TranStackTop(transtack* stack)
{
	return stack->data[stack->length - 1];
}
#define Top TranStackTop

void TranStackPop(transtack* stack)
{
	--stack->length;
}
#define Pop TranStackPop

int TranStackIsEmpty(transtack* stack)
{
	return stack->length == 0;
}
#define IsEmpty TranStackIsEmpty

void hm1(int* a, int size, int p)
{
	while (1)
	{
		int l = p * 2 + 1;
...
void MyNthElement(int* a, int length, int k)
{
	// make first k element as a min heap
	int i;
	for (i = 1; i < k; ++i)
	{
		hm2(a, i + 1, i);
	...
	for (; i < length; ++i)
	{
		// if a[i] is greater than the min of heap, replace the min and 
		if (a[0] < a[i])
		{
			a[0] = a[i];
			hm1(a, k, 0);
...
{
	FILE* fp = fopen("C:\\maxProfit.txt", "w");
	int n = pricesSize - 1;
	int b = 0, s = 0;
	int *profit = malloc(pricesSize / 2 * sizeof(int));
	int profitLength = 0;
	transtack *trans = TransStackCreate(pricesSize / 2);

	// find a new up and save a new profit
	while (s < n)
	{
		// find a new up
		for (b = s; b < n && prices[b] >= prices[b + 1]; ++b);
		for (s = b + 1; s < pricesSize && prices[s - 1] <= prices[s]; ++s);
		--s;

		// while previous transaction(ob, os) satisfy prices[ob] > prices[b]
		// then we will never merge them, just save it as a new profit and ignore it
		while (!IsEmpty(trans) && prices[Top(trans).b] > prices[b])
		{
			profit[profitLength++] = prices[Top(trans).s] - prices[Top(trans).b];
			Pop(trans);
		}
		// while previous transaction(ob, os) satisfy prices[os] <= prices[s] (previous loop confirms prices[ob] <= prices[b])
		// then we can save prices[os] - prices[b] as a new profit and merge these two transaction as a new one
		while (!IsEmpty(trans) && prices[Top(trans).s] <= prices[s])
		{
			profit[profitLength++] = prices[Top(trans).s] - prices[b];
			b = Top(trans).b;
			Pop(trans);
		}
	...
	{
		MyNthElement(profit, profitLength, k);
	}
	int i;
	int totalProfit = 0;
	for (i = 0; i < k; ++i)
	{
		totalProfit += profit[i] > 0 ? profit[i] : 0;
	}
	return totalProfit;
}
